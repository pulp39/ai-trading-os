...
```
操作種別 : WRITE（状態変更）
担い手   : Execution Agent（OpenClaw / Claude Code / Codex / その他承認済みAiiD）
...
| **Institutional** | Collector（OpenClaw） → Proposer（ChatGPT） → Librarian（Claude Code） → Executer | 制度的審議を経た完全フロー |
| **Common** | Librarian（Claude Code） → Executer / Collector | 通常運用タスク |
...
| **Librarian（Claude Code）** | ✅ | ✅（審議参加） | ✅（リポジトリ管理・DB記録・アンカー更新） |
...
| **v2.2** | **2026-04-10** | **P-20260410-002: 役職再編反映（Librarian=Claude Code, Proposer=ChatGPT, Executer=Codex新設）、アクセス権表更新** |
